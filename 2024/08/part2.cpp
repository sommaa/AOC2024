...
int main() {
    ...
    // for each antenna, calculate the distance to the other antennas of the
    // same type
    for (auto &it : map) {
        char c = it.first;
        ...
        for (int i = 0; i < positions.size(); i++) {
            for (int j = i + 1; j < positions.size(); j++) {
                // add the antennas to the anti-nodes matrix
                anti_nodes[positions[i].first][positions[i].second] = true;
                anti_nodes[positions[j].first][positions[j].second] = true;
                // calculate the distance with sign between the two antennas
                int distancex = positions[i].first - positions[j].first;
                int distancey = positions[i].second - positions[j].second;
                for (int k = 1; k < std::max(rows, cols); k++) {
                    std::pair<int, int> anti_node1 =
                        std::make_pair(positions[i].first + distancex * k,
                                       positions[i].second + distancey * k);
                    std::pair<int, int> anti_node2 =
                        std::make_pair(positions[j].first - distancex * k,
                                       positions[j].second - distancey * k);
