...
bool valid(std::map<int, std::vector<int>> &before,
           std::vector<int> &block2_line) {
    bool enabled = true;
    for (int j = 0; j < block2_line.size(); j++) {
        for (int k = j + 1; k < block2_line.size(); k++) {
            ...
            for (int l = 0; l < values.size(); l++) {
                // if the values are not empty, check if the values are equal to
                // the block2[i][k] and if they are, set enabled to false
        ...
        if (!enabled) {
            while (!enabled) {
                for (int j = block2[i].size() - 1; j > 0; j--) {
                    // check if from position j to the end of the block2[i] is
                    // correct
                    std::vector<int> sliced_copy(block2[i].begin() + j - 1,
                                                 block2[i].end());
                    enabled_slice = valid(before, sliced_copy);

                    if (!enabled_slice) {
                        // swap the values of the block2[i][j] and block2[i][j +
                        // 1]
