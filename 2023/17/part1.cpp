...
int shortest_path(const std::vector<std::vector<int>> &data) {
    ...
    while (!hpq.empty()) {
        ...
        // reached destination
        if (r == data.size() - 1 and c == data[0].size() - 1 and n < 3) {
            return heat_loss;
        }
        ...
        if ( n < 3 and (dr != 0 or dc != 0)) {
            int nr = r + dr;
            int nc = c + dc;
            if (nr >= 0 and nr < data.size() and nc >= 0 and nc < data[0].size()) {
        ...
        for (const auto &[ndr, ndc] : directions) {
            if (dr != ndr or dc != ndc) {
                ...
                int nc = c + ndc;
                if (nr >= 0 and nr < data.size() and nc >= 0 and nc < data[0].size()) {
                    hpq.push({heat_loss + data[nr][nc], nr, nc, ndr, ndc, 1});
                }
            }
        }
    }
