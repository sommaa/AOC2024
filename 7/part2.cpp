...
int main() {
    ...
    for (const auto &numbers : matrix) {
        ...
        int n = num_list.size();
        for (int combination = 0; combination < pow(3, n - 1); combination++) {
            long long temp_result = num_list[0];
            for (int i = 1; i < n; i++) {
                int oper = (combination / (int)pow(3, i - 1)) % 3;
                if (oper == 1) { // multiplication
                    temp_result = temp_result * num_list[i];
                } else if (oper == 2) { // addition
                    temp_result = temp_result + num_list[i];
                } else { // concatenation of numbers
                    temp_result =
                        temp_result *
                            pow(10, std::to_string(num_list[i]).length()) +
                        num_list[i];
                }
            }
