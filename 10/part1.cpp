...
int check_neighbors(std::vector<std::vector<int>>& input, int i, int j, std::set<std::pair<int, int>>& visited) {
    // if is already visited, return 0
    ...
    for (auto& dir : directions) {
        ...
        if (nx >= 0 && nx < input.size() && ny >= 0 && ny < input[0].size()) {
            if (input[nx][ny] == 1 + input[i][j]) {
                result += check_neighbors(input, nx, ny, visited);
            }
        }
    }
    return result;
...
int main() {
    ...
    for (int i = 0; i < input.size(); i++) {
        for (int j = 0; j < input[i].size(); j++) {
            if (input[i][j] == 0) {
                // Track visited 9s for this search
                std::set<std::pair<int, int>> visited;

                // Find reachable 9s from this 0
                result += check_neighbors(input, i, j, visited);
            }
        }
    }
